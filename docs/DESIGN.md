...
## ⑩ Moments 活动热力图 (2026-09-25)

概览页底部的全年活动轨迹（GitHub Contributions 的信息可视化方式，Moment 自己的视觉与数据）。

- **指标**：第一版只用「当天有效使用分钟数」；完成任务数进 tooltip。强度阈值集中在 `src/lib/activity.ts`（`calculateActivityLevel`：0 / ≥1 / ≥30 / ≥120 / ≥240 分钟 → level 0–4），UI 不含阈值。
- **数据**：`activity(days?)` 命令一次聚合——屏幕时长循环复用 `total_between`（日界裁剪口径一致，单锁内完成），完成任务拉 `completed_at` 在 Rust 端转本地日分桶；无重复数据系统、无轮询。
- **网格**：365 天对齐周一成 53 列 × 7 行；首尾补齐与未来格子为透明占位；月份标签按真实日期落位（相邻 <3 列跳过）；左侧一/三/五标签。
- **性能**：网格 `useMemo` 一次构建、key 为 ISO 日期；hover（缩放 1.05 + 亮度 + tooltip 淡入 130ms）全部纯 CSS——365 格零 React re-render。
- **视觉**：品牌蓝 `#0a84ff` 4 档透明度阶梯 + level 0 中性底，圆角 2.5px，间距 3px；深浅主题各自定义；「少 ▫▫▫▫▫ 多」图例；响应式靠 `minmax(0,1fr)` 收缩格子（无横向滚动）。
